...
- Removed post-capture recognition overlay from camera screen (sections b, c, d of Capture Process restructured)
- Capture flow simplified: capture → auto-save → return to Art Feed / Capture Collection
- Recognition feedback moved to: (1) push notification, (2) Capture Collection on tap
- Introduced "Recognition Interface" — collaborative artwork identification UX (Duolingo-inspired)
- Notification tab updated: added recognition success/failure notification types with collaborative messaging
- Art Feed (Artwork Feed View): added recognition notification bar + CTA button for unrecognized artworks; artwork title/artist fields show italicized event/timestamp context
- Artwork Detail Screen (Capture Collection): added contextually triggered recognition notification (scroll-triggered by page area); artwork metadata italicized with event name or timestamp for pending captures
- Integration Architecture Flows 1 and 2 updated to reflect async recognition and notification-driven feedback
...
| **Recognition Interface** | A collaborative UI screen that presents available location and timestamp data as starting points and invites the user to confirm or complete artwork identification. Accessible via notification tap or from within Capture Collection. |
...
**b. Post-Capture Behavior (Simplified):**
1. After capture, the image is immediately transmitted to the AI recognition model in the background
2. Artwork is auto-saved to Capture Collection (categorized by check-in context — see Collection Categorization below)
3. Artwork is saved to My Page - Capture List
4. Artwork is saved to personal device photo album
5. User is returned to Art Feed (Home) — the camera closes upon capture
6. Recognition runs asynchronously; result is delivered via notification (see Notification Tab)

> Recognition feedback (success or failure) is no longer shown as an on-camera overlay. The camera experience is kept frictionless — capture and go. Users can interact with recognition results at their own pace via notifications or by opening Capture Collection.
...
**Recognition Status in Artwork Feed:**

For artwork items with **non-success recognition status** (unrecognized or pending):
- A notification bar appears at the **top of the screen** when the user is viewing that artwork's area in the feed
- Notification bar text (collaborative tone):
  - If event name available: *"from [EventName] — Can you help identify this artwork?"*
  - If no event name: *"Captured at [Location] on [Date] — Can you help identify this artwork?"*
- The notification bar is tappable → opens **Recognition Interface**

**Artwork Item Display (unrecognized artworks):**
- **Artwork title field:** Italicized, shows *from [EventName]* (from check-in) OR *captured on [YYYY-MM-DD]* (from timestamp) as placeholder context
- **Artist name field:** Italicized, shows *from [EventName]* OR *captured on [YYYY-MM-DD]* as placeholder context
- **Camera icon + capture count** → replaced with a **CTA button** labeled `"Identify Artwork"` (opens Recognition Interface)

> Location and timestamp serve as preliminary data points — presented as helpful context rather than an error state. The goal is to invite the user to complete the information, not to display a failure.
...
**Recognition Notification (Capture Collection context):**
- As the user scrolls through Artwork Detail pages within Capture Collection, a notification message pops up contextually for **non-success items** (triggered by page area visibility as the user scrolls to each artwork)
- Notification text follows the same collaborative pattern:
  - *"from [EventName] — Can you help identify this artwork?"* OR
  - *"Captured at [Location] on [Date] — Can you help identify this artwork?"*
- Notification is tappable → opens **Recognition Interface**

**Italicized Metadata for Unrecognized Captures:**
- For artworks without confirmed recognition, artwork detail fields display event/timestamp context in *italics*:
  - Artwork name: *from [EventName]* or *captured on [YYYY-MM-DD]*
  - Artist name: *from [EventName]* or *captured on [YYYY-MM-DD]*
- Provides a professional but inviting indication that details are pending user confirmation
...
4. **Post-Capture Flow:**
   - After capture, image is transmitted to AI recognition model in the background
   - Artwork is immediately auto-saved to Capture Collection (categorized by check-in context)
   - Artwork is saved to My Page - Capture List and personal device photo album
   - Camera closes; user returns to Art Feed (Home)
   - Recognition result is delivered asynchronously via push notification at the top of the screen
   - User can choose to interact with the notification or ignore it — notification does not block app usage
   - Tapping the notification opens the **Recognition Interface**

> The camera screen no longer shows a recognition overlay or failure screen. Recognition feedback is non-intrusive and notification-driven. See "Recognition Interface" section and Notification Tab for full detail.
...
The Recognition Interface is a collaborative artwork identification screen that surfaces when recognition requires user attention. It is accessed via:
1. Tapping a recognition notification (success or failure)
2. Tapping an unrecognized artwork inside Capture Collection

**Design Philosophy (Duolingo-inspired, professional tone):**
Rather than showing an error, the interface presents available data (location, timestamp) as helpful starting points and invites the user to confirm or complete artwork details — making the interaction feel collaborative rather than corrective. The tone is friendly but professional: the system is working *with* the user, not failing them.

**On Recognition Success:**
- Displays pre-filled recognized artwork details: title, artist name, year, medium
- Source context line (italicized): `from {EventName}` (from check-in location data) OR `captured on {YYYY-MM-DD}` (from timestamp)
- User can confirm as-is or edit the pre-filled information
- CTA: "Confirm" / "Edit"

**On Recognition Failure / Pending:**
- Collaborative message at top:
  > *"We found this artwork at **[Location]** around **[Time]**. Can you help us identify this artwork?"*
- Pre-fills available metadata as starting points:
  - Source context line (italicized): `from {EventName}` OR `captured on {YYYY-MM-DD}`
  - Artwork name field: awaiting input
  - Artist name field: awaiting input
  - Year and medium fields: optional
- CTA: "Submit" to save manual entry; "Skip for now" to dismiss without input

**API:** Recognition Interface saves manual entries via the existing endpoint `PUT /api/captures/{capture_id}`. No new endpoint required.
...
- **Recognition Success:**
  - Notification text: `"[Artwork Title] by [Artist Name] — recognized from [EventName]"` OR `"[Artwork Title] by [Artist Name] — captured on [Date]"`
  - Tap → opens **Recognition Interface** with confirmed artwork details pre-filled (user can review or edit)

- **Recognition Pending / Failure:**
  - Notification text (collaborative tone): `"We found an artwork at [Location] around [Time]. Help us identify it?"`
  - If event name is available: `"We found an artwork from [EventName]. Can you help identify it?"`
  - Tap → opens **Recognition Interface** with available location/timestamp pre-filled as starting points
  - Notification appears at top of screen (banner style); user can interact or dismiss — does not interrupt navigation
...
8. Frontend immediately auto-saves capture to Capture Collection (categorized by check-in context)
9. Frontend saves to My Page - Capture List and device photo album
10. Camera closes; user returned to Art Feed (Home)
11. On recognition result (async): Backend sends push notification to user
12. User taps notification → opens Recognition Interface to view/confirm recognized details
```
...
5. Django Backend saves capture with "pending" recognition status; returns capture ID to Frontend
6. Frontend auto-saves capture to Capture Collection and returns user to Art Feed (Home)
7. Backend sends recognition failure notification with collaborative message and location/timestamp context
8. User taps notification (or taps capture in Capture Collection) → opens Recognition Interface
9. Recognition Interface displays: "We found this artwork at [Location] around [Time]. Can you help identify it?"
10. User manually enters artwork info (title, artist, year, medium) in collaborative UX
11. Frontend sends manual entry to Django Backend via PUT /api/captures/{capture_id}
12. Django saves capture with manual info; optional: flags for admin review/matching later
```
...
## Version 3.2 (February 18, 2026)

**Updated:**
- Removed in-camera recognition overlay (success/failure) from post-capture flow — camera experience is now frictionless (capture → auto-save → return to Art Feed)
- Recognition feedback is delivered asynchronously via push notification and accessible within Capture Collection
- Introduced **Recognition Interface**: collaborative artwork identification screen with Duolingo-inspired UX; presents location + timestamp as starting points; accessible from notification tap or Capture Collection
- **Notification Tab:** Added recognition success and failure notification types with collaborative messaging
- **Art Feed — Artwork Feed View:** Added contextual recognition notification bar at top for unrecognized artworks; artwork title/artist fields show italicized event/timestamp context; camera icon replaced with "Identify Artwork" CTA button for unrecognized items
- **Artwork Detail Screen (Capture Collection):** Added contextually triggered recognition notification (scroll-triggered by page area); artwork name and artist fields italicized with event name or timestamp for pending captures
- Integration Architecture Flows 1 and 2 updated to reflect async recognition and notification-driven feedback

**No Changes:**
- API endpoints (recognition still uses `POST /api/artworks/recognize` → `/api/v1/artworks/vision-search`; manual entry uses `PUT /api/captures/{capture_id}`)
- Database schema unchanged
- MVP scope, deadlines, and infrastructure unchanged
